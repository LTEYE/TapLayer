...
- **每种按法独立输出方式**：每一层除了"按一下"，还可以设置"连发 N 次"、"按住 N 毫秒"、"按住直到松开触发键"。长按默认输出为按住 1 秒（可自定义）。
...
- **绑定自定义名称**：每个绑定都可以起名字，OSD 提示和绑定卡片显示你的名字（如 `开镜: Ctrl+Shift+S`），不再只显示按键。
- **主题**：浅色 / 深色 / 跟随系统。
- **双语界面**：简体中文和英文。
- **系统托盘**、开机自启（走计划任务，开机不弹权限框）、配置导入导出、单实例防重复启动。
- **自动更新**：启动时自动检查新版本（GitHub 源，超时自动切换 Gitee 源），支持自动下载安装。
...
> **需要管理员权限**：TapLayer 需要向目标程序注入按键，而 Windows 不允许
> 普通权限程序向管理员权限程序发送按键。请**右键以管理员身份运行**。
> 正式版 EXE 已内置"必须管理员"清单，双击即自动以管理员启动（会弹出
> 一次 Windows 权限确认框，属正常现象）。
...
- 每次保存后自动维护一份备份（`config.json.bak`）；配置万一损坏，启动时**自动从备份恢复**，不会悄悄重置成默认。
...
EXE 已内置强制管理员清单（`TapLayer.manifest`），用户双击即弹权限确认框并以管理员启动，无需手动右键。
